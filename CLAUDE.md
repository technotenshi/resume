...
- **TypeScript v7 breaks `vue-tsc` locally**: `vue-tsc@3.3.10` (latest) still triggers `ERR_PACKAGE_PATH_NOT_EXPORTED` with TypeScript 7. `yarn typecheck` will fail locally when run against TS7. This is accepted — typecheck runs with `continue-on-error: true` in CI and does not block builds or deployment. TypeScript is now on `^7.x`; update this note once vue-tsc ships TS7 support.
