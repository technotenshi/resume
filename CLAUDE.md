...
- **`nuxt prepare` required before unit tests on a clean checkout**: `tsconfig.json` extends `.nuxt/tsconfig.json`, which is only generated by `nuxt prepare`. Vitest 4.x's oxc transformer will fail with `TSCONFIG_ERROR: Tsconfig not found` on a fresh clone without it. CI runs `npx nuxt prepare` before `yarn test`; locally, Docker volumes persist `.nuxt/` so this is transparent.
